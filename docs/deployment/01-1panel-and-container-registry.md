...
正式镜像使用 GitHub Container Registry：
...
确认 `main` 已经通过 CI 后创建并推送版本标签：

```bash
git tag -a v0.0.1 -m "Aquafish 0.0.1"
...
- 同时更新 `ghcr.io/yubboo/aquafish:0.0` 和 `latest`；
- 为镜像生成 provenance 和 SBOM。

首次发布后，在 GitHub 的 Packages 页面确认 `aquafish` 容器包可公开读取。
只有公开镜像才能让未登录 GitHub 的 1Panel 服务器直接拉取。
...
```powershell
.\scripts\package-1panel.ps1 -Version 0.0.1
```

产物：

```text
build/1panel/aquafish-0.0.1-1panel.zip
```
